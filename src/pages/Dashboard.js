...
const Dashboard = () => {
  const availableTeamMembers = useMemo(
    () => [
      { value: 'John', label: 'John', profilePic: 'john.jpg', registerNumber: '1234', department: 'Computer Science', mailId: 'john@example.com' },
      { value: 'Alice', label: 'Alice', profilePic: 'alice.jpg', registerNumber: '5678', department: 'Electrical Engineering', mailId: 'alice@example.com' },
      { value: 'Bob', label: 'Bob', profilePic: 'bob.jpg', registerNumber: '91011', department: 'Mechanical Engineering', mailId: 'bob@example.com' },
    ],
    []
  );
  ...
  const handleRemoveTeamMember = (member) => {
    setTeamMembers((prevMembers) => prevMembers.filter((tm) => tm.value !== member.value));
  };

  const handleConfirmSelection = () => {
    if (teamMembers.length === 0 || !selectedMentor) {
      alert('Please select at least one team member and a mentor before confirming.');
      return;
    }
    setConfirmed(true);
          ...
          <div className="card selection-card">
            ...
            <div className="selected-team-members">
              {teamMembers.map((member) => (
                <div key={member.value} className="steam-member">
                  {member.label}
                  {!confirmed && (
                    <button
      ...
      {confirmed && (
        <div className="card confirmation-summary-card">
              ...
              {teamMembers.map((tm) => (
                <div className="team-member" key={tm.registerNumber}>
                  ...
                  <div className="team-member-info">
                    <p><strong>Name:</strong> {tm.name}</p>
                    <p><strong>Register Number:</strong> {tm.registerNumber}</p>
                    <p><strong>Department:</strong> {tm.department}</p>
                    <p><strong>Email:</strong> {tm.mailId}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="mentor-details">
            ...
            <p><strong>Mentor:</strong> {selectedMentor?.label}</p>
          </div>
        </div>
      )}
